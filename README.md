...
### 📝 트러블슈팅 및 기술 문서화
- **핵심 문제 해결**: **단일 IP 라우팅 충돌**, **HTTPS/WebSocket 끊김**, **Ubuntu 오프라인 네트워크 복구** 등 인프라/네트워크 계층의 핵심 문제 해결
...
### 🏗️ 인프라 엔지니어링 및 트러블슈팅
- **Nginx 프록시 설계**: 배포 환경에서 `/uploads` 경로 접근 시 발생한 403/404 권한 오류를 **Nginx `proxy_pass`** 설정으로 해결 
- **환경 변수 표준화**: 로컬(Window/Mac)과 운영 서버(Ubuntu) 간의 파일 시스템 경로 차이를 추상화하여, **배포 환경에서도 이미지가 정상 서빙되도록 구조 개선**
